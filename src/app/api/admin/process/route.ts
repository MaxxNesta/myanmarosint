...
function classifyType(title: string, content: string): EventType {
  ...
  if (/displaced|displacement|refugee|food shortage|hunger|humanitarian|aid block|aid access|civilian.{0,20}kill|hospital.{0,20}bomb|monastery.{0,20}bomb|village.{0,20}burn|burn.{0,20}village|idp/.test(t))
...
export async function GET() {
  ...
  if (articles.length === 0) {
    return NextResponse.json({ message: 'No unprocessed articles', articles: 0 })
  }
  ...
  return NextResponse.json({ articles: articles.length, saved, failed })
}
