  ...
  const filtered = useMemo<MilitaryBase[]>(() => {
    return BASES.filter(b => {
      ...
      if (search) {
        const q = search.trim()
        if (/^\d+$/.test(q)) {
          const regNum = b.regimentEn.replace(/^[A-Z]+ /, '')
          return regNum.startsWith(q)
        }
        const ql = q.toLowerCase()
        return (
          b.regimentEn.toLowerCase().includes(ql) ||
          b.locationEn.toLowerCase().includes(ql) ||
    ...
    <div className="flex flex-col h-[calc(100vh-3.5rem)] overflow-hidden">
      ...
      <div className="bg-surface-1 border-b border-white/[0.07] flex items-center h-10 shrink-0 overflow-x-auto">
        <div className="px-4 h-full border-r border-white/[0.07] shrink-0 flex items-center gap-3">
          ...
          <span className="text-[9px] font-mono text-slate-600 hidden sm:inline">Myanmar Armed Forces · IB ORBAT</span>
        </div>
